...
public class MDownConvert extends IConcreteInstruction {
	private AsmMemoryAddress src = null;
	private ItrAddress dest = null;
	 ...
	 */
	public MDownConvert(AsmMemoryAddress src, ItrAddress dest) {
		super();
		this.src = src;
		this.dest = dest;
	}
	...
	public IInstruction decompose(CompilerStatus cs) {
		...
		// TODO: LOWER - To reduce output file size, use a temporary variable here for dest, and have it close to src 
		...
		ic.addInstruction(new IGoto(dest));
		ic.addInstruction(new IBF("+", 100));
		ic.addInstruction(new IGoto(t.getField(2)));		
		ic.addInstruction(new IBF("]"));
