...
public class MEqual extends IConcreteInstruction {
	AsmMemoryAddress left = null;
	AsmMemoryAddress right = null;
	ItrAddress result = null;
	IInstruction inner = null;
	
	public MEqual(AsmMemoryAddress left, AsmMemoryAddress right, IInstruction inner) {
		this.left = left;
		this.right = right;
		this.inner = inner;
	}
	...
	public IInstruction decompose(CompilerStatus cs) {
		...
		ItrTempAddress three = cs.newTemp(right.getSign()); ic.addInstruction(new ITempLock(three));
		
		ic.addInstruction(new ISet(three, 3));
		ic.addInstruction(new MGtle(left, right, t));
		
		ic.addInstruction(new IReturnPeek());
		ic.addInstruction(new IEqual(t, three, inner));
