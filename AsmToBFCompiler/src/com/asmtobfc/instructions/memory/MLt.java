...
public class MLt extends IConcreteInstruction {
	AsmMemoryAddress left = null;
	AsmMemoryAddress right = null;
	ItrAddress result = null;
	IInstruction inner = null;
	
	public MLt(AsmMemoryAddress left, AsmMemoryAddress right, IInstruction inner) {
		this.left = left;
		this.right = right;
		this.inner = inner;
	}
	...
	// TODO: I am not currently updating the sign bit, for MAdd and MSub.  
	...
	public IInstruction decompose(CompilerStatus cs) {
		...
		ItrTempAddress one = cs.newTemp(right.getSign()); ic.addInstruction(new ITempLock(one));
		
		ic.addInstruction(new ISet(one, 1));
		ic.addInstruction(new MGtle(left, right, t));
		
		ic.addInstruction(new IReturnPeek());
		ic.addInstruction(new IEqual(t, one, inner));
