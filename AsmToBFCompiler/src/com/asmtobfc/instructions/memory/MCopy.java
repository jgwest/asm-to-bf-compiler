...
// TODO: LOWER - Compiler includes >< and <> in the bf result, which is necessarily inefficient

public class MCopy extends IConcreteInstruction {
	AsmMemoryAddress src = null;
	AsmMemoryAddress dest = null;
	
	public MCopy(AsmMemoryAddress src, AsmMemoryAddress dest) {
		this.src = src;
		this.dest = dest;
	}
	...
	public IInstruction decompose(CompilerStatus cs) {
		...
		ic.addInstruction(new ICopy(src.getSign(), dest.getSign()));
		
		for(int  x = 0; x< src.getContents().length; x++) {
			ic.addInstruction(new ICopy(src.getContents()[x], dest.getContents()[x]));
		}
	...
	public String toString() {
		return "[ MCopy " + src.getDebugLabel() + " --> " + dest.getDebugLabel() +" ]";
	}
